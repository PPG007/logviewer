...
import { refreshCacheInfo, refreshConnections, refreshRecent, useStore } from './store/useStore'
...
// 启动即拉取历史文件记录与远端主机（SQLite 持久化）：侧栏在重启后仍可见。
// 注意：只恢复「列表」，不自动重开文件——远端重开需要用户重新输入口令。
refreshRecent().catch(() => {})
