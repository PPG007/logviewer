...
> 本文档按**可验证的里程碑**拆分，每个里程碑有明确产出与验收标准。实施时按 M0 → M10 顺序推进。
...
| M8 | 打磨 + 压测 | 500MB 压测达标 |
| M9 | 远端日志直连（SSH/SFTP） | 连接远端主机、浏览目录、直接打开文件检索 |
| M10 | 远端文件本地内容缓存 | 第二次打开不再走网络；翻页为本地读 |

> 每个里程碑的详细实现（目标、依赖、代码、验收标准、风险）见 [`docs/issues/`](./issues/)（`00`~`10`）。
...
**任务**
- `internal/logfile`：抽出 `Source` 接口（`ReadAt`/`Read`/`Seek`/`Stat`/`Close`），本地文件与远端文件共用同一套索引/检索/分页算法；索引在关闭时能立即中断
- `internal/sshconn`：连接复用（按 `user@host:port`）、密码与私钥认证、主机指纹 TOFU、目录浏览、远端文件字节来源（顺序读走并发快路径、随机读独立句柄）
- `internal/store`：`path_key` 自带来源；新增 `connections` 表（连接配置，不含任何口令）
- `internal/service`：远端 DTO 与 `ConnectRemote`/`ListRemoteDir`/`OpenRemoteFile` 等方法
- 前端：连接表单 + 远端目录浏览器 + 侧栏「远程主机」区
- `internal/testssh`：内嵌 sshd + SFTP 子系统（`os.Root` 限定根目录，支持限速模拟慢链路），让整条链路可端到端单测

**验收**：连接远端主机后能浏览目录并直接打开文件检索/翻页/导出；同名不同主机互不干扰；口令明文保存在本机（可清除，不回传前端）。

### M10 · 远端文件本地内容缓存

**任务**
- `internal/filecache`：复合来源（本地缓存 + 惰性打开的远端来源）、`.part → .bin` 的发布、指纹命中判定、写入者唯一、LRU 逐出、代际号
- `internal/logfile`：`ReadLines` 改为窗口分段读（远端每行一次往返 → 每窗口一次）；`Reload` 改为「先关旧来源再开新的」
- `internal/service`：打开远端时按开关接入缓存；`ReloadFile` 绕过缓存并重写它
- 前端：缓存管理面板、每主机三态开关、目录浏览器「已缓存」标记
- `internal/testssh`：加读字节计数器，用**字节数**而不是计时断言「第二次打开没有传输数据」

**验收**：第二次打开同一份远端内容传输 0 字节且内容一致；内容变化后重下；缓存可查看、可逐条删除、可清空、可按主机/全局关闭。
